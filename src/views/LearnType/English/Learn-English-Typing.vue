<template>
  <div class="learn-english-typing">
    ...
    <!-- Keyborad -->
    <div class="container">
      <keyborad-english v-show="isShowKeyborad"/>
    </div>
    <!-- Settings Model -->
...
// Load English Keyboard Component
import KeyboradEnglish from "@/components/Keyborad-English.vue";
...
export default {
  ...
  components: {
    "keyborad-english": KeyboradEnglish
  },

  beforeRouteEnter(to, from, next) {
    if (to.params.id != undefined) {
      if (store.state.el.lessons !== undefined) {
        var isExistKey = store.state.el.lessons[Base64.decode(to.params.id)];
  ...
  computed: {
    typingContent() {
      var isExistKey =
        store.state.el.lessons[Base64.decode(this.$route.params.id)];
      if (typeof isExistKey !== "undefined") {
        return isExistKey;
      }
      return false;
    ...
    timeDuration() {
      return store.state.el.selectedTimeDuration;
    }
  },

  methods: {
    genTypingContent() {
      this.$store.dispatch("el/generateTypingContent", {
